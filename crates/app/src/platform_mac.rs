...
/// Menubar glyph (36px backing for an 18pt item), template-rendered so
/// macOS recolors it for light/dark/selected states. Regenerated by
/// scripts/make-appicon.sh from bundle/Tomte.icon.
const MENUBAR_ICON: &[u8] = include_bytes!("../assets/menubar@2x.png");
...
impl StatusItem {
    ...
    /// The gnome, as a template image: alpha is the shape, macOS owns the
    /// color. Identity lives here; the title only carries state ("…", "●N").
    fn set_icon(&self, mtm: MainThreadMarker) {
        let Some(button) = self.item.button(mtm) else {
            return;
        };
        let data = NSData::with_bytes(MENUBAR_ICON);
